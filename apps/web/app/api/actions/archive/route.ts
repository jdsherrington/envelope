...
import { runArchiveAction } from "@/lib/server/mail-actions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runArchiveAction(payload.data.accountId, payload.data.threadIds);
    ...
    return serverError(error instanceof Error ? error.message : "Failed to archive threads");
  }
}
