...
import { runRemoveLabelAction } from "@/lib/server/mail-actions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runRemoveLabelAction(
    ...
    return serverError(error instanceof Error ? error.message : "Failed to remove labels");
  }
}
