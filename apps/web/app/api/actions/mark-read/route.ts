...
import { runMarkReadAction } from "@/lib/server/mail-actions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runMarkReadAction(payload.data.accountId, payload.data.threadIds);
    ...
    return serverError(error instanceof Error ? error.message : "Failed to mark read");
  }
}
